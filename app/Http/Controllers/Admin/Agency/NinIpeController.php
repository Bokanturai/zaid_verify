...
class NinIpeController extends Controller
{
    ...
    /**
     * Batch check status for up to 10 nin_ipe requests
     */
    public function checkBatchStatus()
    {
        try {
            ...
            if ($enrollments->isEmpty()) {
                return response()->json([
                    'success' => false,
                    'message' => 'No eligible IPE requests found for batch status check.'
                ], 404);
            }
    ...
    /**
     * Core logic to perform nin_ipe status check via API
     */
    private function performStatusCheck($enrollment)
    {
        try {
            ...
            Log::warning("NinIpe Status Check Failed for ID {$enrollment->id}: " . $lastError);
            return ['success' => false, 'message' => $lastError];

        } catch (\Exception $e) {
            Log::error("NinIpe Status Check Exception for ID {$enrollment->id}: " . $e->getMessage());
    ...
    public function index(Request $request)
    {
        ...
        return view('admin.ninipe.index', compact('enrollments', 'search', 'statusFilter', 'bankFilter', 'statusCounts', 'banks'));
    }
    ...
    public function show($id)
    {
        ...
        return view('admin.ninipe.view', compact('enrollmentInfo', 'statusHistory', 'user'));
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        try {
            ...
            DB::commit();
            return redirect()->route('admin.ninipe.index')
