<x-app-layout>
    <title>Quick Slip - BVN User Management</title>
       <div class="page-body">
    <div class="container-fluid">
      <div class="page-title">
        <div class="row">
          <div class="col-sm-6 col-12">
            <h3 class="mb-0 fw-bold">BVN User Management</h3>
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="row g-3 mb-4">
            ...
            <div class="financial-card shadow-sm h-100 p-4" style="background: var(--info-gradient);">
                <div class="d-flex justify-content-between align-items-start position-relative z-1">
                    <div>
                        ...
                        <small class="text-white-50 fs-12 fw-medium">Currently in work</small>
                    </div>
                    <div class="avatar avatar-lg bg-white bg-opacity-25 rounded-3">
        ...
        <div class="col-xl-3 col-md-6 fade-in-up" style="animation-delay: 0.3s;">
            <div class="financial-card shadow-sm h-100 p-4" style="background: var(--success-gradient);">
                <div class="d-flex justify-content-between align-items-start position-relative z-1">
                    <div>
                        <p class="stats-label mb-1" style="color: white;">Successful</p>
                        <h3 class="stats-value mb-0">{{ $statusCounts['resolved'] ?? 0 }}</h3>
                        <small class="text-white-50 fs-12 fw-medium">Successfully completed</small>
                    </div>
                    <div class="avatar avatar-lg bg-white bg-opacity-25 rounded-3">
        ...
        <div class="col-xl-3 col-md-6 fade-in-up" style="animation-delay: 0.4s;">
            <div class="financial-card shadow-sm h-100 p-4" style="background: var(--danger-gradient);">
                <div class="d-flex justify-content-between align-items-start position-relative z-1">
                    <div>
                        <p class="stats-label mb-1" style="color: white;">Failed</p>
                        <h3 class="stats-value mb-0">{{ $statusCounts['rejected'] ?? 0 }}</h3>
                        <small class="text-white-50 fs-12 fw-medium">Rejected or failed requests</small>
                    </div>
                    <div class="avatar avatar-lg bg-white bg-opacity-25 rounded-3">
        ...
        <div class="col-12">
            <div class="card border-0 shadow-sm overflow-hidden" style="border-radius: 1.25rem;">
                <div class="card-body p-4 bg-white">
                    <form method="GET" action="{{ route('admin.bvn-user.index') }}">
                            ...
                            <div class="col-lg-6">
                                <div class="input-group">
                                    ...
                                    <input type="text" name="search" class="form-control border-start-0 bg-light py-2" 
                                           placeholder="Search BVN, Phone, Name, Ref..." value="{{ request('search') }}">
                                </div>
                            </div>
                            <div class="col-lg-3">
                                <div class="input-group">
                                    ...
                                    <select name="status" class="form-select border-start-0 bg-light py-2" onchange="this.form.submit()">
                                        ...
                                        @foreach(['pending', 'processing', 'successful', 'failed', 'query', 'remark'] as $s)
                            ...
                            <div class="col-lg-3">
                                <div class="d-flex gap-2">
                                    ...
                                    @if(request('status') || request('search'))
                                        <a href="{{ route('admin.bvn-user.index') }}" class="btn btn-outline-danger px-4 py-2 d-flex align-items-center justify-content-center">
                    ...
                    <tbody>
                        @forelse ($enrollments as $enrollment)
                            <tr>
                                ...
                                <td>
                                    <div class="d-flex flex-column">
                                        <span class="fw-semibold text-dark">{{ $enrollment->first_name }} {{ $enrollment->last_name }}</span>
                                        <small class="text-muted">{{ $enrollment->phone_no }}</small>
                                    </div>
                                </td>
                                <td>
                                    <div class="d-flex flex-column">
                                        <span class="text-dark fw-medium small">{{ $enrollment->bank_name ?? 'N/A' }}</span>
                                        <small class="text-primary fw-bold">{{ $enrollment->account_no }}</small>
                                    </div>
                                </td>
                                <td class="text-center">
                                    @php
                                        $statusConfig = match($enrollment->status) {
                                            ...
                                            'successful' => ['color' => 'success', 'icon' => 'ti-circle-check'],
                                            'failed' => ['color' => 'danger', 'icon' => 'ti-circle-x'],
                                ...
                                <td class="pe-4 text-end">
                                    <a href="{{ route('admin.bvn-user.show', $enrollment->id) }}" class="btn btn-icon btn-light btn-sm rounded-circle shadow-sm" data-bs-toggle="tooltip" title="View Details">
                                ...
                                <td colspan="7" class="text-center py-5">
                                    <div class="py-4">
                                        ...
                                        @if(request('search') || request('status'))
                                            <a href="{{ route('admin.bvn-user.index') }}" class="btn btn-link btn-sm mt-2">Clear all filters</a>
                                        @endif
                                    </div>
                                </td>
                            </tr>
